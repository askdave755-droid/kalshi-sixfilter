...
signals_log = []
daily_stats = {"pnl": 0, "trades": 0, "date": datetime.now().date()}
...
async def toggle_auto():
    ...
    status_msg = "ENABLED" if auto_scan_enabled else "DISABLED"
    
    # Send test message if enabling
    if auto_scan_enabled:
        await send_telegram_alert(f"✅ <b>Kalshi SixFilter Activated</b>\n\nAuto-scanner: {status_msg}\nMonitoring markets every 5 minutes...")
    
    return {
        "success": True,
        "auto_scan": auto_scan_enabled,
        "message": f"Auto-scanner {status_msg}",
    ...
    <script>
        ...
        async function toggleAuto() {
            try {
                ...
                const scannerIndicator = document.getElementById('scannerStatus');
                scannerIndicator.innerHTML = data.auto_scan ? 
        ...
        async function checkStatus() {
            try {
                ...
                if (!data.telegram_set) {
                    ...
                    telegramDiv.textContent = '⚠️ Telegram not configured. Add TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID to Railway.';
                } else {
                    telegramDiv.className = 'alert alert-success';
                    telegramDiv.textContent = '✅ Telegram configured. Ready for alerts.';
                }
