...
class KalshiClient:
    ...
    def _sign(self, method: str, path: str, body: str = "") -> Dict[str, str]:
        ...
        signature = base64.b64encode(
            hmac.new(self.private_key.encode("utf-8"), msg.encode("utf-8"), hashlib.sha256).digest()
        ).decode("utf-8")
        return {
...
class SixFilterEngine:
    ...
    def ev_gap(self, edge: float) -> FilterResult:
        return FilterResult(edge >= MIN_EDGE_CENTS, min(100, (edge / max(MIN_EDGE_CENTS, 0.1)) * 50), f"Edge={edge:.1f}c")
    ...
    def bayesian(self, hour: int, daily_pnl: float) -> FilterResult:
        ...
        return FilterResult(9 <= hour <= 15, 80 if 9 <= hour <= 15 else 30, f"Hour={hour}, PnL=${daily_pnl:.2f}")
    
    def stoikov(self, ask: float, bid: float) -> FilterResult:
        spread = ask - bid
        return FilterResult(spread <= 5, max(0, 100 - spread * 10), f"Spread={spread:.1f}c")
    
    def analyze(self, ticker: str, side: str, yes_ask: float, no_ask: float, model_prob: float, daily_pnl: float) -> Dict:
        ...
        f6 = self.stoikov(yes_ask, 100 - yes_ask)
...
def dashboard():
    ...
    for pos in s["positions"]:
        ...
        positions.append({**pos, "market_price": round(mkt_price, 1), "unrealized_pnl": round(unreal, 2)})
